...
class MbMap extends React.Component {
      ...
      >
        ...
        <div style={{ position: "relative" }}>
          <div class="map-cont-controls" ref={this.mapContControls}>
            ...
            <div ref={this.underlayBGCont} style={{ marginBottom: "8px" }}>
              {/* Hide the underlay controls by default with an "advanced" link */}
              <div
                ref={(el) => {
                  this.underlayShowDiv = el;
                }}
                style={{
                  textAlign: "right",
                  marginBottom: "-3px",
        ...
        >
          ...
          <label
            className="map-slider-item"
            ref={(el) => {
              this.mapSliderLabel = el;
            }}
            style={{ width: "3em", textAlign: "center" }}
          >
            {new Date(
              this.state.maxDate || ConfirmedMapFns.getToday()
            ).getDate() +
              "/" +
              (new Date(
                this.state.maxDate || ConfirmedMapFns.getToday()
        ...
        <span className="due">
          ...
          >
            ...
            >
              The time slider selects the current day. The "7/14/21 days"
              controls show the current day's value minus the value that many
              days ago.
            </li>
            <li style={{ color: "#555" }}>
    ...
    map.on("dragend", async (e) => {
      try {
        dvAna({
          ...
          dateSlider: new Date(my.state.maxDate).toISOString(),
        });
      } catch (e) {
        return null;
      }
  ...
  _updateMode() {
    ...
    this.statesAndTerritories.forEach((stateName) => {
      ...
      let absGeoBoundariesInst = absDataSource
          ? this.getGeoBoundariesInst(stateName, "lga")
          : null,
        caseGeoBoundariesInst = this.getGeoBoundariesInst(
          stateName,
          casesDataSource.schema
        );

      if (caseGeoBoundariesInst)
        caseGeoBoundariesInst.addCasesFillPoly(casesDataSource);
      if (absGeoBoundariesInst)
        absGeoBoundariesInst.addABSStatsFillPoly(absDataSource, otherMaxMin);

      if (caseGeoBoundariesInst)
        caseGeoBoundariesInst.addLinePoly(
          casesDataSource,
          "rgba(0, 0, 0, 1.0)"
        );
      if (absGeoBoundariesInst)
        absGeoBoundariesInst.addLinePoly(absDataSource, "rgba(0, 0, 0, 0.1)");

      if (caseGeoBoundariesInst)
        caseGeoBoundariesInst.addHeatMap(casesDataSource);
    });
  ...
  _resetMode(prevState) {
    ...
    this.statesAndTerritories.forEach((stateName) => {
      ...
      var absGeoBoundariesInst = absDataSource
          ? this.getGeoBoundariesInst(stateName, "lga")
          : null,
        caseGeoBoundariesInst = this.getGeoBoundariesInst(
          stateName,
